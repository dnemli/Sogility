        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          ...
          <label className="flex flex-col gap-2">
            ...
            <select
              ...
              className="h-11 min-w-[160px] rounded-2xl border border-slate-200/80 bg-white/90 px-4 text-sm font-medium text-slate-900 shadow-sm outline-none focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100"
            >
        ...
        <div className="h-[300px] w-full min-w-0">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={data} margin={{ top: 8, right: 8, left: -8, bottom: 0 }}>
              ...
              />
              <Tooltip
                cursor={{ fill: "rgba(29, 143, 90, 0.06)" }}
                formatter={(value: number) => [`${value} players`, "Unique players"]}
              />
              <Bar dataKey="uniquePlayers" name="Unique players" fill="#1d8f5a" radius={[10, 10, 0, 0]} maxBarSize={48} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
